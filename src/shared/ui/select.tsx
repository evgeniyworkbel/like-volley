...
{
  /** @todo:  удалить и прокидывать items через пропс */
}
const fonts = [
...
export default function ExampleSelect() {
  /** @todo:   убрать default и переназвать компонент с ExampleSelect на Select (селект из библиотеки можешь в импорте переименовать с Select на BaseSelect) */
  return (
    <Select.Root items={fonts}>
      ...
      <Select.Portal>
        <Select.Positioner className="z-10 outline-none select-none" sideOffset={8}>
          <Select.Popup className="group text-gray-900 shadow-gray-200 outline-gray-200 dark:outline-gray-300 min-w-(--anchor-width) origin-(--transform-origin) rounded-md bg-[canvas] bg-clip-padding shadow-lg outline-1 transition-[transform,scale,opacity] data-ending-style:scale-90 data-ending-style:opacity-0 data-starting-style:scale-90 data-starting-style:opacity-0 data-[side=none]:min-w-[calc(var(--anchor-width)+1rem)] data-[side=none]:data-ending-style:transition-none data-[side=none]:data-starting-style:scale-100 data-[side=none]:data-starting-style:opacity-100 data-[side=none]:data-starting-style:transition-none dark:shadow-none">
...
function ChevronUpDownIcon(props: React.ComponentProps<"svg">) {
  /** @todo:   убрать отсюда иконки и положить в icons папку  */
  return (
    <svg
      width="8"
      height="12"
