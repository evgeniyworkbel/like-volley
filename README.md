...
### До деплоя
- если был изменен конфиг cms или связанные с ним файлы, то в конце работы следует запустить скрипт `generate:importmap` и `generate:types`. Также в конце работы над фичей следует запустить скрипт `migrate:create` (чтобы создать миграции) и закоммитить
